...
async def startup_event():
    ...
    # Multi-segment LanSearch relay (SDK broadcast -> unicast to extra subnets)
    try:
        from miloco_server import lan_relay
        relay_thread = threading.Thread(target=lan_relay.main, daemon=True)
        relay_thread.start()
        logger.info("LAN relay thread started")
    except Exception as e:
        logger.error("LAN relay thread failed to start: %s", e)
