...
class MIoTLan:
    ...
    def __init__(
        ...
        self._lan_devices = {}
        # Extra /24 subnets to unicast-scan (multi-segment LAN, e.g. Keenetic IOT2)
        self._extra_subnets = [s.strip() for s in (extra_subnets or []) if s.strip()]
        self._last_extra_scan_ts = 0
    ...
    def __scan_extra_subnets(self) -> None:
        """Unicast-probe every /24 in extra_subnets (multi-segment LAN support).

        Broadcast (255.255.255.255) does not cross router segments; cameras in
        another segment (e.g. Keenetic IOT2 192.168.2.x) must be probed directly.
        """
        if not self._extra_subnets:
            return
        ts_now = int(time.time())
        if ts_now - self._last_extra_scan_ts < self.OT_PROBE_INTERVAL_MIN:
            return
        self._last_extra_scan_ts = ts_now
        for subnet in self._extra_subnets:
            try:
                parts = subnet.split(".")
                if len(parts) != 4:
                    _LOGGER.error("invalid extra subnet, %s", subnet)
                    continue
                base = ".".join(parts[:3])
                for host in range(1, 255):
                    try:
                        self.ping_internal(target_ip=f"{base}.{host}")
                    except Exception as err:  # pylint: disable=broad-exception-caught
                        _LOGGER.debug("extra subnet ping error, %s: %s", host, err)
                _LOGGER.info("extra subnet scanned, %s", subnet)
            except Exception as err:  # pylint: disable=broad-exception-caught
                _LOGGER.error("extra subnet scan error, %s, %s", subnet, err)
